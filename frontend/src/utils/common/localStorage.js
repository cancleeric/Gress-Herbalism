...
export function hasCompletedHerbalismTutorial() {
  try {
    ...
    console.warn('無法從 localStorage 讀取本草教學狀態:', e);
    return false;
  }
}
...
export function setHerbalismTutorialCompleted(completed = true) {
  try {
    localStorage.setItem(STORAGE_KEYS.HERBALISM_TUTORIAL_COMPLETED, String(completed));
  } catch (e) {
    console.warn('無法儲存本草教學狀態到 localStorage:', e);
  }
}
